...
public class MoneyChangerFrame extends JFrame {
	...
	class MyPanel extends JPanel {
		...
		private JCheckBox[] check = new JCheckBox[unit.length - 1]; //1원 제외한 체크박스
			...
			calcBtn.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					for (int i = 0; i < unit.length; i++) {
						if (i == unit.length - 1) { // 1원짜리 계산
							res = money / unit[i];
							tf[i].setText(Integer.toString(res));
							break;
						}
						if (!check[i].isSelected()) { //체크가 안되어있다면 0을 보여줌
							tf[i].setText("0");
							continue;
						}
